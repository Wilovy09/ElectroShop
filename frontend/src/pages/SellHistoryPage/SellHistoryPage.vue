...
async function getHistoryElements() {
  ...
  try {
    ...
    else sales.value = await sellHistoryRepository.getSellHistory()
  } catch (e) {
    handleError(e)
  } finally {
...
<template>
  ...
  <div v-else class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
    ...
    <div
      v-for="sale in sales"
      :key="sale.id_sell"
      class="rounded-xl bg-slate-800 p-4 text-white shadow-md transition-shadow duration-300 hover:shadow-lg"
    >
      <h3 class="mb-2 text-xl font-semibold">{{ sale.product_name }}</h3>
      ...
      <div class="mt-4 flex justify-end">
        <RouterLink :to="{ name: 'product', params: { id: sale.id_product } }">
          <button class="text-zinc-400 transition-colors duration-300 hover:text-white">
            Ver detalles
          </button></RouterLink
        >
      </div>
